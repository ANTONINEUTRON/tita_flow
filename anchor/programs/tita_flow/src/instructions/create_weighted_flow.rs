...
// Structure to define a weighted allocation
#[derive(AnchorSerialize, AnchorDeserialize, Clone)]
pub struct WeightedAllocation {
    pub recipient: Pubkey,      // Recipient who will receive funds
    pub weight: u32,            // Weight in basis points (10000 = 100%)
    pub description: Option<String>, // Optional description of this allocation
}
...
pub struct CreateWeightedFlow<'info> {
    #[account(mut)]
    ...
    /// CHECK: The allocation vault accounts are initialized in the instruction handler
    /// These must be provided in the same order as the allocations vector
    #[account(mut)]
    pub allocation_vaults: UncheckedAccount<'info>,
...
impl<'info> CreateWeightedFlow<'info> {
